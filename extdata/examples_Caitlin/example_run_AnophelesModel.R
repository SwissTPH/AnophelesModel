...
unique(activity_patterns$country)
# To include custom vector biting patterns you will need to modify the $HBI and $HBO vectors of activity_p
# IMPORTANT: HBI, HBO, humans_indoors, humans_in_bed should have the same length and their values should correspond to the same time points.
...
activity_p_stephensi$HBI <- c()
activity_p_stephensi$HBO <- c()
activity_p_stephensi$humans_indoors <- c()
activity_p_stephensi$humans_in_bed <- c()
activity_p_stephensi$humans_outdoors <- c()
