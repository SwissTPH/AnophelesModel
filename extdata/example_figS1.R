...
working_dir = "~/GitRepos/STPHrepos/AnophelesModel/extdata/om_data_new/"
...
db_exp_KEN = "~/OpenMalaria/AnophelesModel2/KEN.sqlite"
db_exp_PNG = "~/OpenMalaria/AnophelesModel2/PNG.sqlite"
scens_KEN = readRDS("~/OpenMalaria/AnophelesModel2/KEN/cache/scenarios.rds")
scens_PNG = readRDS("~/OpenMalaria/AnophelesModel2/PNG/cache/scenarios.rds")
...
levels(plot_df$setting) <- c(levels(plot_df$setting), "An. gambiae (Kenya)",
                             "An. farauti (Papua New Guinea)")
plot_df[which(plot_df$setting == "KEN"), "setting"] = "An. gambiae (Kenya)"
plot_df[which(plot_df$setting == "PNG"), "setting"] = "An. farauti (Papua New Guinea)"

plot_df$setting = factor(plot_df$setting, levels = c("An. farauti (Papua New Guinea)",
                                                     "An. gambiae (Kenya)"))

# Make the dates relevant for the present time
plot_df$date = as.Date(plot_df$date) %m+% years(23)
plot_df = plot_df[which(format(as.Date(plot_df$date, format="%d/%m/%Y"),"%Y") <2029),]

ggplot(plot_df, aes(x = as.Date(date), y = mean_prev*100, color = setting)) +
  theme_light() + theme_linedraw() + theme_bw(base_size = 16) +
  geom_ribbon(aes(ymin = (mean_prev - sd_prev)*100,
                  ymax = (mean_prev + sd_prev)*100,
                  fill = setting, alpha = 0.5, color = NULL), show.legend = FALSE) +
    geom_vline(xintercept = as.Date("2023-01-01"), color = "black", linetype = "dashed") +
    geom_vline(xintercept = as.Date("2025-01-01"), color = "black", linetype = "dashed") +
...
ggsave(paste0(out_dir, "Fig4_revision.pdf"), width = 8, height = 4)
